...
export class IpcBusCommonClient extends EventEmitter
    ...
    send(channel: string, ...args: any[]) {
        this._ipcBusTransport.ipcPushCommand(IpcBusUtils.IPC_BUS_COMMAND_SENDMESSAGE, {}, {channel: channel, sender: this._ipcBusTransport.ipcBusSender}, args);
    }
    ...
    addListener(channel: string, listener: IpcBusInterfaces.IpcBusListener): this {
        ...
        this._ipcBusTransport.ipcPushCommand(IpcBusUtils.IPC_BUS_COMMAND_SUBSCRIBE_CHANNEL, {}, {channel: channel, sender: this._ipcBusTransport.ipcBusSender});
        return this;
    }

    removeListener(channel: string, listener: IpcBusInterfaces.IpcBusListener): this {
        super.removeListener(channel, listener);
        this._ipcBusTransport.ipcPushCommand(IpcBusUtils.IPC_BUS_COMMAND_UNSUBSCRIBE_CHANNEL, {}, {channel: channel, sender: this._ipcBusTransport.ipcBusSender});
        return this;
    }
    ...
    removeAllListeners(channel?: string): this {
        if (channel) {
            this._ipcBusTransport.ipcPushCommand(IpcBusUtils.IPC_BUS_COMMAND_UNSUBSCRIBE_CHANNEL, {unsubscribeAll: true}, {channel: channel, sender: this._ipcBusTransport.ipcBusSender});
